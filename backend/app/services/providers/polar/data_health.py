"""Polar health data implementation for sleep, recovery, and activity samples.

Syncs data from Polar AccessLink API v3 endpoints:
- /v3/users/sleep                          → EventRecord (category=sleep) + SleepDetails
- /v3/users/nightly-recharge               → DataPointSeries (recovery_score, HRV)
- /v3/users/continuous-heart-rate/{date}   → DataPointSeries (heart_rate)
- /v3/users/activities                     → DataPointSeries (steps, energy)
"""
...
class PolarHealthData(Base247DataTemplate):
    """Polar implementation for health data (sleep, recovery, activity)."""
    ...
    def normalize_sleep(
        ...
        # Polar provides durations in minutes, convert to seconds
        ...
        awake_seconds = max(0, duration_seconds - light_sleep_sec - deep_sleep_sec - rem_sleep_sec)
    ...
    # ------------------------------------------------------------------
    # Activity Samples — /v3/users/activities/samples (required by base)
    # Implemented as a no-op since Polar does not have a multi-type samples
    # endpoint equivalent to Suunto; HR is handled via continuous-heart-rate.
    # ------------------------------------------------------------------
    ...
    def load_and_save_all(
        ...
        # 1. Sleep → EventRecord + SleepDetails
        try:
        ...
        # 2. Nightly Recharge → DataPointSeries (recovery_score, HRV)
        try:
